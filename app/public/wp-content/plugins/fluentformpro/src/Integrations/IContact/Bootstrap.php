...
//        add_filter('fluentform_notifying_async_icontact', '__return_false');
    ...
    public function saveGlobalSettings($settings)
    {
        ...
        try {
            ...
            return wp_send_json_success([
                'status'  => true,
                'message' => __('Your API key settings has been verfied and updated. All good!', 'fluentformpro')
            ], 200);
    ...
    public function getMergeFields($list, $listId, $formId)
    {
        ...
        $customFields = $api->get_custom_fields($listId);
    ...
    {
        $api = new IContactApi(
            $settings['appKey'], $settings['username'], $settings['apiPassword'], $settings['clientFolderId']
        );
    ...
    */
    public function notify($feed, $formData, $entry, $form)
    {
        ...
        if($contactId) {
            // contact has been added or updated.
            // now we have to add the subscription
            $result = $this->addSubscription($contactId, $feedData['list_id']);
            if($result) {
                do_action('ff_log_data', [
                    ...
                    'description' => 'iContact feed has been successfully initialed and pushed data'
                ]);
            }
        }
        ...
        do_action('ff_log_data', [
            ...
            'status' => 'failed',
            'title' => $feed['settings']['name'],
            'description' => 'iContact Data not submitted'
        ]);
    }
    ...
    private function syncContact($contact)
    {
        ...
        if ($is_new_contact) {
            try {
                $new_contact = $api->add_contact($contact);

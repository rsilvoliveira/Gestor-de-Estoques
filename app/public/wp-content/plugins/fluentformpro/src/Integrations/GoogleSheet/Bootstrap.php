...
//        add_filter('fluentform_notifying_async_google_sheet', '__return_false');
    ...
    public function getGlobalFields($fields)
    {
        ...
        return [
            ...
            'menu_description' => __('Copy that Google Access Code from other window and paste it here, then clik on Verify Code button.', 'fluentformpro'),
    ...
    public function saveGlobalSettings($settings)
    {
        ...
        wp_send_json_success([
            'message' => __('Your Google Sheet api key has been verfied and successfully set', 'fluentformpro'),
            'status' => true
        ], 200);
    }
    ...
    public function checkColumnSlugs($settings, $integrationId)
    {
        ...
        if (count($settings['meta_fields']) > 104) {
    ...
    public function notify($feed, $formData, $entry, $form)
    {
        ...
        foreach ($metaFields as $field) {
            $row[] = esc_html(strip_tags(ArrayHelper::get($field, 'item_value')));
        }
        ...
        if (is_wp_error($response)) {
            ...
            do_action('ff_log_data', [
                ...
                'status' => 'success',
                'title' => $feed['settings']['name'],
                'description' => 'Google Sheet feed has been successfully initialed and pushed data'
            ]);
        }
    }
}

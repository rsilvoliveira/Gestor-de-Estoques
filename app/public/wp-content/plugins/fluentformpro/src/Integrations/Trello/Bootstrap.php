    ...
    */
    ...
    public function notify($feed, $formData, $entry, $form)
    {
        ...
        if (!is_wp_error($response) && isset($response['id'])) {
            do_action('ff_log_data', [
                ...
                'description'      => 'Trello feed has been successfully initialed and pushed data'
            ]);
        } else {
            // trello card adding failed
            do_action('ff_log_data', [
                'parent_source_id' => $form->id,
                'source_type'      => 'submission_item',
                'source_id'        => $entry->id,
                ...
                'status'           => 'failed',
                'title'            => $feed['settings']['name'],
                'description'      => is_wp_error($response) ? $response->get_error_messages() : 'API Error when submitting Data in trello server'
            ]);
        }
    }
    ...
    protected function getApiClient()
    {
        ...
        return new TrelloApi(
            $settings['accessToken'], $listId = null
        );
    }
    ...
    {
        return $list;
        // TODO: Implement getMergeFields() method.
    }
}

...
//        add_filter('fluentform_notifying_async_platformly', '__return_false');
        ...
        return [
            'fields'              => [
                ...
                [
                    ...
                    'label'        => 'Tags',
                    'placeholder'  => 'Select Platformly Segment',
                    'tips'         => 'Associate tags to your Platformly contacts with a comma separated list (e.g. new lead, FluentForms, web source). Commas within a merge tag value will be created as a single tag.',
    ...
    protected function getTags()
    {
        ...
        foreach ($tags as $tag) {
            if (is_array($tag)) {
                $formattedLists[$tag['id']] = $tag['name'];
            }
        }
    ...
    public function notify($feed, $formData, $entry, $form)
    {
        ...
        if (!is_wp_error($response) && $response['status'] === 'success') {
            ...
            do_action('ff_log_data', [
                ...
                'description'      => 'Platformly feed has been successfully initialed and pushed data'
            ]);

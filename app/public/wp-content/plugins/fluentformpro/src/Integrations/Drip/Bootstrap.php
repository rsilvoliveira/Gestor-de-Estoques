...
class Bootstrap extends IntegrationManager
{
    ...
    public function notify($feed, $formData, $entry, $form)
    {
        ...
        if ($customFields = ArrayHelper::get($feedData, 'custom_fields')) {
            ...
            if ($customData) {
                $customData['custom_fields'] = $customData;
            }
        }

        $tags = array_filter(explode(",", $feedData['tags']));
        if ($tags) {
            $addData['tags'] = array_map('trim', $tags);
        }
        ...
        if (!is_wp_error($response)) {
            do_action('ff_log_data', [
                ...
                'description' => 'Drip feed has been successfully initialed and pushed data'
            ]);
        } else {

    ...
    public function notify($feed, $formData, $entry, $form)
    {
        ...
        if (!is_wp_error($response) && !empty($response['id'])) {
            ...
            do_action('ff_log_data', [
                ...
                'description'      => 'SendinBlue feed has been successfully initialed and pushed data'
            ]);
        } else {
          $error_log =   $response->get_error_messages()[0];
            ...
            do_action('ff_log_data', [
                ...
                'status'           => 'failed',
                'title'            => $feed['settings']['name'],
                'description'      => is_wp_error($response) ? $error_log : 'API Error when submitting Data'
            ]);
        }
    }

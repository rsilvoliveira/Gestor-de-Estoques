...
class FormStylerGenerator
{
    ...
    public function generateSubmitButton($style, $selector)
    {
        ...
        if ($stylesAllignment) {
            $stylesAllignment = "{$selector} .ff_submit_btn_wrapper { {$stylesAllignment} }";
        }
    ...
    public function generateInputStyles($item, $selector)
    {
        ...
        $normalCss = $this->generateNormal($normalStyles, '');
        if ($normalCss) {
            $normalCss = "{$selector} .ff-el-input--content input, .ff-el-input--content textarea, {$selector} .ff-el-input--content select, .ff-el-input--content .select2 .selection ul { {$normalCss} }";
        }
        $focusStyles = Arr::get($item, 'all_tabs.tabs.focus.value');
        $focusCss = $this->generateNormal($focusStyles, '');
        if ($focusCss) {
            $focusCss = "{$selector} .ff-el-input--content input:focus, .ff-el-input--content textarea:focus, {$selector} .ff-el-input--content select:focus { {$focusCss} }";
        }

...
class PaymentMethods extends BaseFieldManager
{
    ...
    public function __construct()
    {
        ...
        add_filter('fluentform_editor_init_element_payment_method', array($this, 'recheckEditorComponent'));
    }

    function getComponent()
    {
        $available_methods = apply_filters('fluentformpro_available_payment_methods', array());
    ...
    public function recheckEditorComponent($component)
    {
        ...
        $existingMethods = ArrayHelper::get($component, 'settings.payment_methods', array());
    ...
    function render($data, $form)
    {
        ...
        foreach ($paymentMethods as $methodName => $paymentMethod) {
            if(ArrayHelper::get($paymentMethod, 'enabled') == 'yes' && isset($available_methods[$methodName])) {
                $activatedMethods[$methodName] = $paymentMethod;
            }
        }

        if(!$activatedMethods) {
            echo wp_sprintf('<p class="ff-error ff-payment-method-error">%s</p>', __('No activated payment method found. If you are an admin please check the payment settings', 'fluentformpro'));
            return;
        }
        ...
        if(count($activatedMethods) == 1) {
            ...
            $data['attributes']['value'] = key($activatedMethods);

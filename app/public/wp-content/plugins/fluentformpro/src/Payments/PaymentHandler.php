...
class PaymentHandler
{
    public function init()
    {
        ...
        add_action('wp_ajax_handle_payment_ajax_endpoint', [$this, 'handleAjaxEndpoints']);
        ...
        add_action('fluentform_rending_payment_form', function ($form) {
            ...
            wp_localize_script('fluentform-payment-handler', 'fluentform_payment_config', [
                'i18n' => [],
                'stripe' => [
                    'publishable_key' => StripeSettings::getPublishableKey()
                ]
            ]);
    ...
    public function maybeHandlePayment($insertData, $data, $form)
    {
        ...
        if (FormFieldsParser::hasElement($form, 'payment_method') && !$paymentAction->selectedPaymentMethod) {
            wp_send_json_error([
                'message' => [__('Sorry! No selected payment method found. Please select a valid payment method', 'fluentformpro')]
            ], 423);
        }

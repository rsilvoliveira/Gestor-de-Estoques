...
class PaymentReceipt
{
    ...
    public function getSubmissionValue($property)
    {
        if($property == 'payment_total') {
            return OrderData::calculateOrderItemsTotal($this->getOrderItems(), true, $this->entry->currency);
        }

        return ucfirst($this->entry->{$property});
    }
    ...
    private function paymentInfo()
    {
        ...
        return $this->loadView('payment_info', array(
            'submission' => $this->entry,
            'items' => $orderItems,
            'orderTotal' => OrderData::calculateOrderItemsTotal($orderItems, true, $this->entry->currency)
        ));
    }

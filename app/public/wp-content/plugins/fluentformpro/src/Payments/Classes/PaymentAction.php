...
class PaymentAction
{
    ...
    private function setupData()
    {
        ...
        foreach ($formFields as $fieldKey => $field) {
            if(in_array(ArrayHelper::get($field, 'element'), $paymentInputElements)) {
                $paymentInputs[$fieldKey] = $field;
            } else  if(ArrayHelper::get($field, 'element') == 'item_quantity_component') {
                $quantityItems[ArrayHelper::get($field, 'settings.target_product')] = ArrayHelper::get($field, 'attributes.name');
            } else if(ArrayHelper::get($field, 'element') == 'payment_method') {
                $paymentMethod = $field;
            }
        }
